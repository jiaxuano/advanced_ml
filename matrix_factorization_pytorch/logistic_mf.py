...
def train_one_epoch(model, train_df, optimizer):
    ...
    loss.backward()
    optimizer.step()
    # train_loss = valid_metrics(model, )
    ### END SOLUTION
    return loss.item()
...
def valid_metrics(model, valid_df):
    ...
    y_hat_label = y_hat.round()
    corrects = (predicted_classes == ratings).float()
    valid_acc = corrects.sum() / len(ratings)
    ### END SOLUTION
